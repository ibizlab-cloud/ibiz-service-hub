...
public class DEServiceAPIRuntime extends net.ibizsys.central.dataentity.service.DEServiceAPIRuntime implements IDEServiceAPIRuntime {
	...
	@Override
	public Object createDownloadTicket(String strScope, IDEServiceAPIRSRuntime iDEServiceAPIRSRuntime, String strParentKey, String strKey, String strStorageField, String strOSSFileId, Object objTag) throws Throwable {
		testAccessUser();
		try {
			return this.onCreateDownloadTicket(strScope, iDEServiceAPIRSRuntime, strParentKey, strKey, strStorageField, strOSSFileId, objTag);
		} catch (Throwable ex) {
			if (ex instanceof DEServiceAPIRuntimeException) {
				throw (DEServiceAPIRuntimeException) ex;
			}

			throw new DEServiceAPIRuntimeException(this, String.format("建立文件下载凭证发生异常，%1$s", ex.getMessage()), ex);
		}
	}
	
	protected Object onCreateDownloadTicket(String strScope, IDEServiceAPIRSRuntime iDEServiceAPIRSRuntime, String strParentKey, String strKey, String strStorageField, String strOSSFileId, Object objTag) throws Throwable {
		if (!(this.getDataEntityRuntime() instanceof net.ibizsys.central.cloud.core.dataentity.IDataEntityRuntime)) {
			throw new Exception("实体运行时对象类型不正确");
		}
		...
		Object key = this.getSystemRuntime().convertValue(this.getDataEntityRuntime().getKeyPSDEField().getStdDataType(), strKey);
		return realDataEntityRuntime.createDownloadTicket(key, strStorageField, strOSSFileId, false);
	}
	...
	protected Object onFetchDataSet(String strScope, IDEServiceAPIRSRuntime iDEServiceAPIRSRuntime, String strParentKey, IPSDEServiceAPIMethod iPSDEServiceAPIMethod, Object objBody, String strKey, Object objTag) throws Throwable {
		...
		if (this.getDataEntityRuntime().isEnableVersionControl() || this.getDataEntityRuntime().isEnableVersionStorage()) {
	...
	protected Object doExportData(String strExportTag, Object objData, OutputStream outputStream) throws Throwable {
		...
		if(iDEDataExportRuntime instanceof net.ibizsys.central.cloud.core.dataentity.dataexport.IDEDataExportRuntime) {
			return ((net.ibizsys.central.cloud.core.dataentity.IDataEntityRuntime)this.getDataEntityRuntime()).exportData2(strExportTag, objData, outputStream);
		}
