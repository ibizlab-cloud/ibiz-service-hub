	...
	public Mono<Void> sendMessage(JSONRPCMessage message) {
		...
		try {
			...
			if(message instanceof JSONRPCRequest) {
				JSONRPCRequest jsonPCRequest = (JSONRPCRequest)message;
				if(jsonPCRequest.getParams() instanceof net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.Request) {
					Map<String, Object> map = ((net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.Request)jsonPCRequest.getParams()).getHeaders();
			...
			Future<SimpleHttpResponse> future = httpClient.execute(request, new FutureCallback<SimpleHttpResponse>() {
				...
				public void completed(SimpleHttpResponse response) {
					//logger.debug("completed sending message");
				}
