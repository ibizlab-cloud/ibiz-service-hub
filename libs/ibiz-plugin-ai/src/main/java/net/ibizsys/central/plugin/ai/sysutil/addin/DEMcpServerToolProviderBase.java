...
public abstract class DEMcpServerToolProviderBase extends McpServerToolProviderBase {
	...
	private RecursiveCharacterTextSplitter deMethodResultSplitter = null;

	@Override
	protected void onInit() throws Exception {

		if (this.getContext().getDEMethodMaxTokens() > 0) {
			this.deMethodResultSplitter = new RecursiveCharacterTextSplitter(null, this.getContext().getDEMethodMaxTokens(), 0);
		}
	...
	protected SyncToolSpecification getGetEntityJsonSchemaSyncToolSpecification() throws Exception {
		...
		SyncToolSpecification syncToolSpecification = new SyncToolSpecification(new Tool("get_entity_json_schema", sb.toString(), strToolSchema), (exchange, arguments) -> {
	...
	protected SyncToolSpecification getGetEnumSyncToolSpecification() throws Exception {
		...
		SyncToolSpecification syncToolSpecification = new SyncToolSpecification(new Tool("get_enum", sb.toString(), strToolSchema), (exchange, arguments) -> {
	...
	protected SyncToolSpecification getListEntityMethodsSyncToolSpecification() throws Exception {
		...
		for (IDEServiceAPIRuntime iDEServiceAPIRuntime : this.getDEServiceAPIRuntimes()) {
			...
			} else if (StringUtils.hasLength(iDEServiceAPIRuntime.getDataEntityRuntime().getPSDataEntity().getMemo())) {
		...
		SyncToolSpecification syncToolSpecification = new SyncToolSpecification(new Tool("list_entity_methods", sb.toString(), strToolSchema), (exchange, arguments) -> {
			String strEntityName = arguments.get("entity_name").toString();
			...
			if (!ObjectUtils.isEmpty(psDEServiceAPIMethodList)) {
				for (IPSDEServiceAPIMethod iPSDEServiceAPIMethod : psDEServiceAPIMethodList) {
					if (SADEMethodType.DEACTION.value.equals(iPSDEServiceAPIMethod.getMethodType())) {
						...
						if (StringUtils.hasLength(iPSDEServiceAPIMethod.getCodeName())) {
							node.put("name", iPSDEServiceAPIMethod.getCodeName());
						} else {
						...
						} else if (StringUtils.hasLength(iPSDEAction.getMemo())) {
				...
				for (IPSDEServiceAPIMethod iPSDEServiceAPIMethod : psDEServiceAPIMethodList) {
					if (SADEMethodType.FETCH.value.equals(iPSDEServiceAPIMethod.getMethodType())) {
						...
						if (StringUtils.hasLength(iPSDEServiceAPIMethod.getMemo())) {
							node.put("description", iPSDEServiceAPIMethod.getMemo());
						} else if (StringUtils.hasLength(iPSDEDataSet.getMemo())) {
						...
						if (iPSDEDataSet.getPSDEDataSetReturn() != null) {
							...
							return_node.put("type", "object");
							// return_node.put("$ref",
							// String.format("%1$s.json",
							// iDEServiceAPIRuntime.getDataEntityRuntime().getPSDataEntity().getCodeName()));
							// return_node.put("array", true);
	...
	protected SyncToolSpecification getExecuteEntityMethodSyncToolSpecification() throws Exception {
		...
		SyncToolSpecification syncToolSpecification = new SyncToolSpecification(new Tool("execute_entity_method", sb.toString(), strToolSchema), (exchange, arguments) -> {
			String strEntityName = arguments.get("entity_name").toString();
	...
	protected String executeEntityMethod(String strEntityName, String strMethodName, Object input) throws Throwable {
		...
		try {
			...
			String strRet =  convertExecuteEntityMethodResult(ret, iDEServiceAPIRuntime, strMethodName, input);
			if(StringUtils.hasLength(strRet) && this.deMethodResultSplitter!=null ) {
				List<String> items = this.deMethodResultSplitter.splitText(strRet);
				if(items.size()>1) {
					return items.get(0) + " ...。注意：由于长度限制，内容被截断!";
				}
			}
	...
	protected Object onExecuteEntityMethod(IDEServiceAPIRuntime iDEServiceAPIRuntime, String strMethodName, Object input) throws Throwable {
		...
		if (input instanceof Map) {
			...
			IPSDEServiceAPIMethod iPSDEServiceAPIMethod = iDEServiceAPIRuntime.getPSDEServiceAPIMethod(strMethodName, true);
			if(iPSDEServiceAPIMethod != null) {
				if(SADEMethodType.FETCH.value.equals(iPSDEServiceAPIMethod.getMethodType())) {
					try {
						JsonNode jsonNode =	JsonUtils.toJsonNode(input);
						if(jsonNode.isArray()) {
							body = JsonUtils.asList(input);
							strKey = null;
						}
						else
							if(jsonNode.isObject()) {
								body = JsonUtils.asMap(input);
								strKey = null;
							}
					}
					catch (Throwable ex) {
						log.error(String.format("解析输入[%1$s]到Body发生异常，%2$s", input, ex.getMessage()), ex);
					}
				}
			}
		}
