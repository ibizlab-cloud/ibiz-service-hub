...
import net.ibizsys.central.plugin.ai.mcp.server.McpStatelessServerFeatures.AsyncToolSpecification;
...
import net.ibizsys.central.plugin.ai.mcp.spec.McpStatelessServerTransport;
...
public class McpStatelessAsyncServer {
	...
	public Mono<Void> addTool(McpStatelessServerFeatures.AsyncToolSpecification toolSpecification) {
		...
		AsyncToolSpecification wrappedToolSpecification = withStructuredOutputHandling(this.jsonSchemaValidator, toolSpecification);
	...
	private List<ResourceTemplate> getResourceTemplates() {
		...
		List<ResourceTemplate> resourceTemplates = this.resources.keySet()
			...
			.map(uri -> {
				 ...
				 ResourceTemplate template = new ResourceTemplate(resource.uri(), resource.name(), resource.title(),
	...
	private McpStatelessRequestHandler<McpSchema.CompleteResult> completionCompleteRequestHandler() {
		return new McpStatelessRequestHandler<McpSchema.CompleteResult>() {

			@Override
			public Mono<CompleteResult> handle(McpTransportContext ctx, Object params) {
				...
				if (type.equals("ref/prompt") && request.ref() instanceof McpSchema.PromptReference) {
					...
					McpStatelessServerFeatures.AsyncPromptSpecification promptSpec = McpStatelessAsyncServer.this.prompts
				...
				if (type.equals("ref/resource") && request.ref() instanceof McpSchema.ResourceReference) {
					...
					McpStatelessServerFeatures.AsyncResourceSpecification resourceSpec = McpStatelessAsyncServer.this.resources
				...
				McpStatelessServerFeatures.AsyncCompletionSpecification specification = McpStatelessAsyncServer.this.completions.get(request.ref());
