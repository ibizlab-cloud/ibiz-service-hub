...
import net.ibizsys.central.plugin.ai.mcp.spec.DefaultMcpStreamableServerSessionFactory;
import net.ibizsys.central.plugin.ai.mcp.spec.JsonSchemaValidator;
import net.ibizsys.central.plugin.ai.mcp.spec.JsonSchemaValidator.ValidationResponse;
import net.ibizsys.central.plugin.ai.mcp.spec.McpClientSession;
...
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.CompleteResult;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.LoggingLevel;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.LoggingMessageNotification;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.Prompt;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.Resource;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.ResourceTemplate;
import net.ibizsys.central.plugin.ai.mcp.spec.McpSchema.SetLevelRequest;
...
import net.ibizsys.central.plugin.ai.mcp.spec.McpServerTransportProviderBase;
import net.ibizsys.central.plugin.ai.mcp.spec.McpStreamableServerTransportProvider;
import net.ibizsys.central.plugin.ai.mcp.util.Assert;
import net.ibizsys.central.plugin.ai.mcp.util.DeafaultMcpUriTemplateManagerFactory;
import net.ibizsys.central.plugin.ai.mcp.util.McpUriTemplateManagerFactory;
...
//	}
	private Map<String, McpNotificationHandler> prepareNotificationHandlers(McpServerFeatures.Async features) {
	    ...
	    if (Utils.isEmpty(rootsChangeConsumers)) {
	        rootsChangeConsumers = Collections.singletonList(
	            new BiFunction<McpAsyncServerExchange, List<McpSchema.Root>, Mono<Void>>() {
	                @Override
	                public Mono<Void> apply(McpAsyncServerExchange exchange, List<McpSchema.Root> roots) {
	                    return Mono.fromRunnable(new Runnable() {
	                        @Override
	                        public void run() {
	                            logger.warn("Roots list changed notification, but no consumers provided. "
	                                + "Roots list changed: {}", roots);
	                        }
	                    });
	                }
	            }
		...
		public Mono<CallToolResult> apply(McpAsyncServerExchange exchange, McpSchema.CallToolRequest request) {
			...
			return this.delegateCallToolResult.apply(exchange, request).map(result -> {
				...
				if (Utils.isEmpty(result.content())) {
					...
					return new CallToolResult(Arrays.asList(new McpSchema.TextContent(validation.jsonStructuredOutput())),
	...
	private static List<McpServerFeatures.AsyncToolSpecification> withStructuredOutputHandling(
		...
		return tools.stream()
		        ...
		        .collect(Collectors.toList()); // JDK 8 兼容的收集操作
	}
	...
	private McpRequestHandler<McpSchema.ListResourcesResult> resourcesListRequestHandler() {
		return (exchange, params) -> {
			Object resourceList = this.resources.values()
				.stream()
				.map(McpServerFeatures.AsyncResourceSpecification::resource)
				 .collect(Collectors.toList());
			return Mono.just(new McpSchema.ListResourcesResult((List<Resource>)resourceList, null));
		};
	}
	...
	private List<McpSchema.ResourceTemplate> getResourceTemplates() {
		List list = new ArrayList<>(this.resourceTemplates);
		List<ResourceTemplate> resourceTemplates = this.resources.keySet()
			...
			.map(uri -> {
				...
				McpSchema.ResourceTemplate template = new McpSchema.ResourceTemplate(resource.uri(), resource.name(), resource.title(),
	...
	private McpRequestHandler<McpSchema.CompleteResult> completionCompleteRequestHandler() {
		return new McpRequestHandler<McpSchema.CompleteResult>() {

			@Override
			public Mono<CompleteResult> handle(McpAsyncServerExchange exchange, Object params) {
				...
				if (type.equals("ref/prompt") && request.ref() instanceof McpSchema.PromptReference) {
					...
					McpServerFeatures.AsyncPromptSpecification promptSpec = McpAsyncServer.this.prompts.get(promptReference.name());
					if (promptSpec == null) {
				...
				if (type.equals("ref/resource") && request.ref() instanceof McpSchema.ResourceReference ) {
					...
					McpServerFeatures.AsyncResourceSpecification resourceSpec = McpAsyncServer.this.resources.get(resourceReference.uri());
				...
				McpServerFeatures.AsyncCompletionSpecification specification = McpAsyncServer.this.completions.get(request.ref());
...
//	}
	private McpSchema.CompleteRequest parseCompletionParams(Object object) {
	    ...
	    // 1. 兼容 Java 8 的传统 switch 语句（替代 switch 表达式）
	    switch (refType) {
	        case "ref/prompt":
	            String title = refMap.get("title") != null ? (String) refMap.get("title") : null;
	            ref = new McpSchema.PromptReference(refType, (String) refMap.get("name"), title);
	            break;
	        case "ref/resource":
