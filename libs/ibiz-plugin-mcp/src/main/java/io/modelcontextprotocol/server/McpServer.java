...
public interface McpServer {
	...
	class AsyncSpec {
		 ...
		 */
		public McpAsyncServer build() {
			List tools = this.tools.stream().map(McpServerFeatures.AsyncToolRegistration::toSpecification).collect(Collectors.toList());

			Map resources = this.resources.entrySet()
				.stream()
				.map(entry -> new AbstractMap.SimpleEntry<>(entry.getKey(), entry.getValue().toSpecification()))
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

			Map prompts = this.prompts.entrySet()
				.stream()
				.map(entry -> new AbstractMap.SimpleEntry<>(entry.getKey(), entry.getValue().toSpecification()))
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

			List rootsChangeHandlers = this.rootsChangeConsumers.stream()
	...
	class SyncSpec {
		 ...
		 */
		public McpSyncServer build() {
			List tools = this.tools.stream().map(McpServerFeatures.SyncToolRegistration::toSpecification).collect(Collectors.toList());
