...
public class MysqlEndOfMonthFunction extends SQLFunctionBase {

	public final static MysqlEndOfMonthFunction DEFAULT = new MysqlEndOfMonthFunction();
	...
	public SqlCall parse(SqlBasicCall sqlBasicCall, Map<String, Object> params) {
		...
		try {
			...
			String sql = String.format("SELECT TIMESTAMP(DATE_FORMAT(LAST_DAY(%1$s), '%%Y-%%m-%%d 23:59:59')) FROM t1", sqlBasicCall.getOperandList().get(0).toSqlString(Db2SqlDialect.DEFAULT).getSql());
