...
public class MysqlYearWeekCharFunction extends SQLFunctionBase {

	public final static MysqlYearWeekCharFunction DEFAULT = new MysqlYearWeekCharFunction();
	...
	public SqlCall parse(SqlBasicCall sqlBasicCall, Map<String, Object> params) {
		...
		try {
			...
			String sql = String.format("SELECT CONCAT(YEAR(%1$s), 'W', LPAD(WEEK(%1$s), 2, '0')) FROM t1", sqlBasicCall.getOperandList().get(0).toSqlString(Db2SqlDialect.DEFAULT).getSql());
