...
public abstract class SysEAIAgentRuntimeBase extends net.ibizsys.runtime.res.SysDataSyncAgentRuntimeBase implements ISysEAIAgentRuntime {
	...
	protected void onPrepareDefaultSetting() throws Exception {
		...
		if (!StringUtils.hasLength(strTopic) && StringUtils.hasLength(this.getPSSysDataSyncAgent().getCodeName())) {
	...
	protected void onRecv(String strTopic, String strData) throws Throwable {
		this.onRecv(strTopic, strData, new HashMap<String, Object>());
	}
	...
	public synchronized void registerSysEAIMsgListener(ISysEAIMsgListener iSysEAIMsgListener) {
		...
		if (!SysDataSyncAgentDirs.IN.equals(this.getPSSysDataSyncAgent().getSyncDir()) && !SysDataSyncAgentDirs.INOUT.equals(this.getPSSysDataSyncAgent().getSyncDir())) {
