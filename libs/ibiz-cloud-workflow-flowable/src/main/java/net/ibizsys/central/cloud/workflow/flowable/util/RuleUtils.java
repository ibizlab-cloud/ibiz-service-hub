...
public class RuleUtils
{
    ...
    public static String COLUMN_ISNULL = "isnull";
    public static String COLUMN_ISNOTNULL = "isnotnull";
    public static String COLUMN_IN = "in";
    public static String COLUMN_NOTIN = "not in";
    public static String COLUMN_LIKE = "like";

    public static boolean test(String scope, Object param) {
        if(ObjectUtils.isEmpty(scope) || ObjectUtils.isEmpty(param)){
            return false;
        }
        ...
        if (matcher.find()) {
            ...
            throw new RuntimeException("scope解析错误");
        }
        ...
        if (conditions.size() >= 3) {
          //  throw new BadRequestAlertException(String.format("暂不支持该配置[%s]", scope), "", "");
        	  throw new RuntimeException(String.format("暂不支持该配置[%s]", scope));
        } else if (conditions.size() > 0) {
            while (conditions.size() > 0) {
                parseField(sbf, conditions.pop());
            }
        }

        if (!ObjectUtils.isEmpty(sbf.toString()) && !ObjectUtils.isEmpty(param)) {
            EvaluationContext ctx = new StandardEvaluationContext();
            ctx.setVariable("dto", param);
            Expression exp = parser.parseExpression(sbf.toString());
            return exp.getValue(ctx, Boolean.class);
        }
        return false;
    }

    private static void parseField(StringBuffer sbf, String condition) {
        String[] args = condition.split(";");
        if (args.length != 3) {

            //throw new BadRequestAlertException(String.format("scope解析错误:%s", args.toString()), "", "");
        	throw new RuntimeException(String.format("scope解析错误:%s", args.toString()));
        }

        String field = args[0].replace("'", "");
        String exp = args[1].replace("'", "");
        String fileValue = args[2].replace("'", "");

        if (COLUMN_EQ.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"EQ\",\"%s\")", field, fileValue));
        } else if (COLUMN_NE.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"NOTEQ\",\"%s\")", field, fileValue));
        } else if (COLUMN_GT.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"GT\",\"%s\")", field, fileValue));
        } else if (COLUMN_GE.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"GE\",\"%s\")", field, fileValue));
        } else if (COLUMN_LT.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"LT\",\"%s\")", field, fileValue));
        } else if (COLUMN_LE.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"LE\",\"%s\")", field, fileValue));
        } else if (COLUMN_ISNULL.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"ISNULL\",\"\")", field));
        } else if (COLUMN_ISNOTNULL.equalsIgnoreCase(exp)) {
            sbf.append(String.format("T(net.ibizsys.central.cloud.workflow.flowable.util.RuleUtils).test(#dto.get(\"%s\"),\"ISNOTNULL\",\"\")", field));
        } else {
           // throw new BadRequestAlertException(String.format("暂不支持该配置[%s]", condition), "", "");
        	throw new RuntimeException(String.format("暂不支持该配置[%s]", condition));
        }
    }
	...
	public static Object getObj(Object object, String members)
	{
		...
		for (int j = 0; j < i; ++j)
		{
			...
			try
			{
				if(currentObj instanceof IEntity)
				{
					currentObj=((IEntity) currentObj).get(methodName);
					if (currentObj == null) {
						return null;
					}
				}
...
//				}
	...
	public static boolean equal(Object exp, Object finalObject)
	{
		...
		{
			try {
				Timestamp tm= DataTypeUtils.getDateTimeValue(exp,null);
	...
	public static boolean gt(Object exp, Object finalObject)
	{
        ...
        try
        {
            if(finalObject instanceof java.sql.Timestamp)
            {
				...
				{
					tm= DataTypeUtils.getDateTimeValue(exp,null);
					if(tm==null){
						return false;
					}
				}
            ...
            {
                return ((Integer) finalObject) > DataTypeUtils.getIntegerValue(exp,Integer.MAX_VALUE);
            }
            else if (finalObject instanceof Double)
            {
                return ((Double) finalObject) > DataTypeUtils.getDoubleValue(exp,Double.MAX_VALUE);
            }
            else if (finalObject instanceof Float)
            {
                return ((Float) finalObject) > DataTypeUtils.getFloatValue(exp,Float.MAX_VALUE);
            }
			else if (finalObject instanceof BigDecimal)
			{
				return ((BigDecimal) finalObject).compareTo(DataTypeUtils.getBigDecimalValue(exp,BigDecimal.valueOf(Double.MAX_VALUE)))>0;
			}
			else if (finalObject instanceof BigInteger)
			{
				return ((BigInteger) finalObject).compareTo(DataTypeUtils.getBigIntegerValue(exp,BigInteger.valueOf(Long.MAX_VALUE)))>0;
			}
			else if (finalObject instanceof Long)
			{
				return ((Long) finalObject) > DataTypeUtils.getLongValue(exp,Long.MAX_VALUE);
			}
