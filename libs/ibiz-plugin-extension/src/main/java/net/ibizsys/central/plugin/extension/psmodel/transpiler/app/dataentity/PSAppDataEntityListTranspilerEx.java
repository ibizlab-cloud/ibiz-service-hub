...
public class PSAppDataEntityListTranspilerEx extends net.ibizsys.model.util.transpiler.extend.app.dataentity.PSAppDataEntityListTranspilerEx {
			...
			if(iPSApplication!=null) {
				...
				if(!ObjectUtils.isEmpty(psAppDataEntities)) {
					for(IPSAppDataEntity iPSAppDataEntity : psAppDataEntities) {
						...
						if(strDETag.equalsIgnoreCase(fullId)) {
							fullId = iExtensionPSModelRTServiceSession.getPSModelUniqueTag(iPSAppDataEntity);
