...
public class PSAppDEACModeListTranspilerEx extends net.ibizsys.model.util.transpiler.extend.app.dataentity.PSAppDEACModeListTranspilerEx {
				...
				if(!ObjectUtils.isEmpty(psAppDataEntities)) {
					for(IPSAppDataEntity iPSAppDataEntity : psAppDataEntities) {
						...
						if(strDETag.equalsIgnoreCase(strParentId)) {
							//获取指定自填模式（仅校验）
							IPSAppDEACMode iPSAppDEACMode = iPSAppDataEntity.getPSAppDEACMode(getSimpleId(fullId), false);
							super.onGetModelRef(iPSModelTranspileContext, PSModelUtils.calcUniqueTag2(iPSAppDEACMode), childMode, objectNode);
							return;
						}
					}
				}
