...
public class PSSysBICubeMeasureRTService extends net.ibizsys.psmodel.runtime.service.PSSysBICubeMeasureRTService {
	...
	protected Object doInvoke(String methodName, String key, Object params) throws Exception {
   		...
   		if(METHOD_APPLY.equalsIgnoreCase(methodName)) {
   			PSSysBICubeMeasure domain = this.getDomain(params);
	...
	@Override
	protected PSSysBICubeMeasure doGet(String key, boolean tryMode) throws Exception {
		...
		if(StringUtils.hasLength(iExtensionPSModelRTServiceSession.getExtensionId())) {
			...
			searchContextDTO.eq(V2SystemExtensionBICube.FIELD_TYPE, V2SystemExtensionBICubeItemType.MEASURE.value);
			...
			if(!ObjectUtils.isEmpty(page) && !ObjectUtils.isEmpty(page.getContent())) {
				List<PSSysBICubeMeasure> psSysBICubeMeasureList = ExtensionUtils.toPSModelList(page.getContent(), V2SystemExtensionBICube.FIELD_ITEM_TAG, PSSysBICubeMeasure.class);
				if(!ObjectUtils.isEmpty(psSysBICubeMeasureList)) {
					return psSysBICubeMeasureList.get(0);
				}
			}
		}
	...
	protected void onBeforeCreate(PSSysBICubeMeasure m) throws Exception {
		...
		if(StringUtils.hasLength(strPSSysBICubeId) && strPSSysBICubeId.indexOf("@")!=-1) {
			m.setPSSysBICubeMeasureId(String.format("%1$s@%2$s", strPSSysBICubeId.split("[@]")[0], m.getPSSysBICubeMeasureId()));
		}
	}
	
	@Override
	protected void doCreate(PSSysBICubeMeasure m) throws Exception {
		...
		ObjectNode objectNode = iExtensionPSModelRTServiceSession.getPSModelTranspiler(IPSSysBICubeMeasure.class, false).compile(iExtensionPSModelRTServiceSession, m, null);
   		...
   		IPSSysBICubeMeasure iPSSysBICubeMeasure = iExtensionPSModelRTServiceSession.getPSSystemService().createAndInitPSModelObject((IPSModelObjectRuntime)iPSSysBICube, IPSSysBICubeMeasure.class, objectNode);
   		//验证模型
   		PSModelMergeUtils.verify(iPSSysBICubeMeasure, true);
	...
	@Override
	protected List<PSSysBICubeMeasure> getDomainList(String strDataSetName, PSSysBICubeMeasureFilter f) throws Exception {
		...
		List<PSSysBICubeMeasure> list = super.getDomainList(strDataSetName, f);
		IExtensionPSModelRTServiceSession iExtensionPSModelRTServiceSession = (IExtensionPSModelRTServiceSession)this.getPSModelRTServiceSession();
		if(!StringUtils.hasLength(iExtensionPSModelRTServiceSession.getExtensionId())) {
			return list;
		}
		...
		if(true) {
			...
			if(!ObjectUtils.isEmpty(page) && !ObjectUtils.isEmpty(page.getContent())) {
				List<PSSysBICubeMeasure> psSysBICubeMeasureList = ExtensionUtils.toPSModelList(page.getContent(), V2SystemExtensionBICube.FIELD_ITEM_TAG, PSSysBICubeMeasure.class);
				list = ExtensionUtils.replacePSModelList(list, psSysBICubeMeasureList, PSSysBICubeMeasure.class);
			}
		}
	...
	@Override
	protected PSSysBICubeMeasure getLast(PSSysBICubeMeasure m) throws Exception {
		return this.get(m.getId(), false);
	}
	
	@Override
	protected void doMergeLast(PSSysBICubeMeasure m, PSSysBICubeMeasure last) throws Exception {
	...
	@Override
	protected void doUpdate(PSSysBICubeMeasure m) throws Exception {
	...
	public Object apply(PSSysBICubeMeasure m) throws Exception {
   		return this.doApply(m);
   	}

   	protected Object doApply(PSSysBICubeMeasure m) throws Exception {
   		
   		IExtensionPSModelRTServiceSession iExtensionPSModelRTServiceSession = (IExtensionPSModelRTServiceSession) this.getPSModelRTServiceSession();
   		String strExtensionId = iExtensionPSModelRTServiceSession.getExtensionIdMust();
   		PSSysBICubeMeasure psSysBICubeMeasure = this.get(m.getId(), false);
   		String strPSDynaInstId = (String)psSysBICubeMeasure.get("psdynainstid");
   		if(!StringUtils.hasLength(strPSDynaInstId)) {
   			throw new Exception("无法应用非动态扩展指标");
   		}
   	
   		ObjectNode objectNode = iExtensionPSModelRTServiceSession.getPSModelTranspiler(IPSSysBICubeMeasure.class, false).compile(iExtensionPSModelRTServiceSession, psSysBICubeMeasure, null);
   		
   		
   		IPSModelRTService iPSSysBICubeService = (IPSModelRTService)this.getPSModelRTServiceSession().getPSModelService(PSModels.PSSYSBICUBE);
   		IPSSysBICube iPSSysBICube = (IPSSysBICube)iPSSysBICubeService.getPSModelObject(psSysBICubeMeasure.getPSSysBICubeId(), IPSSysBICube.class, false);
   		IPSSysBICubeMeasure iPSSysBICubeMeasure = iExtensionPSModelRTServiceSession.getPSSystemService().createAndInitPSModelObject((IPSModelObjectRuntime)iPSSysBICube, IPSSysBICubeMeasure.class, objectNode);
   		//验证模型
   		PSModelMergeUtils.verify(iPSSysBICubeMeasure, true);
		...
		V2SystemExtensionBICube v2SystemExtensionBICube = new V2SystemExtensionBICube();
		v2SystemExtensionBICube.setExtensionModel(ExtensionUtils.toExtensionModel(psSysBICubeMeasure));
   	...
   	@Override
   	protected void doRemove(PSSysBICubeMeasure m) throws Exception {
   		IExtensionPSModelRTServiceSession iExtensionPSModelRTServiceSession = (IExtensionPSModelRTServiceSession) this.getPSModelRTServiceSession();
   		String strExtensionId = iExtensionPSModelRTServiceSession.getExtensionIdMust();
   		PSSysBICubeMeasure psSysBICubeMeasure = this.get(m.getId(), false);
   		String strPSDynaInstId = (String)psSysBICubeMeasure.get("psdynainstid");
